"""
Services for working with wishlists
"""
...
async def delete_item_image(item_id: int, user: User) -> WishlistItem:
    """
    Delete an image of user's item with item_id
    """
    wishlist = await user.wishlist
...
async def reserve(
    ...
    if item.reserved_user:
        raise HTTPException(
            ...
            detail="Item already reserved")
    if item.wishlist == await user.wishlist:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Can't reserve your own item")
...
async def cancel_reservation(
    ...
    if item.reserved_user != user:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You didn't reserve this item")
